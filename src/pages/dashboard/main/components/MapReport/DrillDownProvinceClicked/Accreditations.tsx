...
import { IAccreditationDrillDown } from "@/interfaces/api/hospital"
...
export default function Accreditations(props: propsInterface) {
    ...
    const [hospitals, setHospitals] = useState<IAccreditationDrillDown[]>([])
    ...
    const fetchData = async () => {
        try {
            const res = await XverServices.HospitalsWithAccreditations(
            ...
            const sortedAccreditation = res.data.sort((a: IAccreditationDrillDown, b: IAccreditationDrillDown) => {
                if (!a.score.Valid && !b.score.Valid) return -1
                return b.score.Float64 - a.score.Float64
            })
            setHospitals(sortedAccreditation)
        } catch (err: any) {
            console.log(err)
            setLoading(prevState => ({
                ...prevState,
        ...
        <>
...

            {
                ...
                <div className={`w-full flex flex-col`}>
                    ...
                    <div 
                        className={`
                            w-full flex flex-col overflow-y-auto custom_scrollbar_show
                        `}
                        style={{
                                    ...
                                    >
                                        ...
                                        <div className={`flex flex-center`}>
                                            <Badge
                                                variant="filled"
                                                style={{
                                                    backgroundColor: item.color || colors.N_A,
                                                    color: isBrightColor(item.color, 140) ? 'black' : 'white',
                                                }}
                                            >
                                                { 
                                        ...
                                        <p className={`text-center`}>
                                            { getAnyValueShowToColumn(item.incident_rate) }
                                        </p>
                                        <p className={`text-center`}>
                                            {/* @ts-ignore */}
                                            { getAnyValueShowToColumn(item.occupy_bed_rate) }
                                        </p>
                                    </div>
                                )
                            })
