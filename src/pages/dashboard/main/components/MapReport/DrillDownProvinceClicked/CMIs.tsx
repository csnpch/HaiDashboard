...
import { ICMIsDrillDown } from "@/interfaces/api/hospital"
import { Tooltip } from "@mantine/core"
import { Badge } from "@mantine/core"
import { XverServices } from "@/services/api/hospital/dashboard"
import { _class, isBrightColor } from "@/utils/helpers/functions"
...
export default function CMIs(props: propsInterface) {
    ...
    const [hospitals, setHospitals] = useState<ICMIsDrillDown[]>([])
    ...
    const fetchData = async () => {
        try {
            const res = await XverServices.HospitalsWithCMIs(
            ...
            const sortedCMI = res.data.sort((a: ICMIsDrillDown, b: ICMIsDrillDown) => {
                return b.cmi - a.cmi
            })
            setHospitals(sortedCMI)
        } catch (err: any) {
            console.log(err)
            setLoading(prevState => ({
                ...prevState,
                                    ...
                                    >
                                        ...
                                        <div className={`flex flex-center`}>
                                            <Badge
                                                variant="filled"
                                                style={{
                                                    backgroundColor: item.cmi_color || colors.N_A,
                                                    color: isBrightColor(item.cmi_color) ? 'black' : 'white',
                                                }}
                                            >
                                                {/* @ts-ignore */}
                                                { 
                                        ...
                                        <p className={`text-center`}>
                                            {/* @ts-ignore */}
                                            { 
                                                item.bed_occupy_rate.toFixed(2)
                                            }
                                        </p>
                                    </div>
                                )
