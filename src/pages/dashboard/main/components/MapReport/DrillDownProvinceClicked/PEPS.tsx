...
import { IPePsDrillDown } from "@/interfaces/api/hospital"
...
export default function PEPS(props: propsInterface) {
    ...
    const [hospitals, setHospitals] = useState<IPePsDrillDown[]>([])
    ...
    const fetchData = async () => {
        try {
            const res = await XverServices.PEPSDrillDown(
            ...
            const sortedPEPS = res.data.sort((a: IPePsDrillDown, b: IPePsDrillDown) => {
                if (!a.peps.Valid || !b.peps.Valid) return -1
                return b.peps.Float64 - a.peps.Float64
            })
            setHospitals(sortedPEPS)
        } catch (err: any) {
            console.log(err)
            setLoading(prevState => ({
                ...prevState,
                                    ...
                                    >
                                        ...
                                        <div className={`flex flex-center`}>
                                            <Badge
                                                variant="filled"
                                                style={{
                                                    backgroundColor: item.peps.Valid ? item.color : colors.N_A,
