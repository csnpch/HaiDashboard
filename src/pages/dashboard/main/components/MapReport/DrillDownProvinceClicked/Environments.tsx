...
import { IEnvironmentsDrillDown } from "@/interfaces/api/hospital"
...
export default function Environments(props: propsInterface) {
    ...
    const [hospitals, setHospitals] = useState<IEnvironmentsDrillDown[]>([])
    ...
    const fetchData = async () => {
        try {
            const res = await XverServices.HospitalsWithEnvironments(
            ...
            const sortedEnvSafeties = res.data.sort((a: IEnvironmentsDrillDown, b: IEnvironmentsDrillDown) => {
                if (!a.environment_percentage.Valid && !b.environment_percentage.Valid) return -1
                return b.environment_percentage.Float64 - a.environment_percentage.Float64
            })
            setHospitals(sortedEnvSafeties)
        } catch (err: any) {
            console.log(err)
            setLoading(prevState => ({
                ...prevState,
                                    ...
                                    >
                                        ...
                                        <div className={`flex flex-center`}>
                                            <Badge
                                                variant="filled"
                                                style={{
                                                    backgroundColor: item.environment_percentage.Valid ? item.color : colors.N_A,
                                            ...
                                            >
                                                {/* @ts-ignore */}
                                                {
                                                    getAnyValueShowToColumn(
                                                        item.environment_percentage.Valid 
                                                        ? item.environment_percentage.Float64.toFixed(2)
                                                        : 'N/A'
                                                    )
                                                }
                                            </Badge>
                                        ...
                                        <p className={`text-center`}>
                                            {/* @ts-ignore */}
                                            { 
                                                getAnyValueShowToColumn(item.infection_rate)
                                            }
                                        </p>
                                    </div>
                                )
