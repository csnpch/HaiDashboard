...
import { IMortalitiesDrillDown } from "@/interfaces/api/hospital"
...
export default function Mortalities(props: propsInterface) {
    ...
    const [hospitals, setHospitals] = useState<IMortalitiesDrillDown[]>([])
    ...
    const fetchData = async () => {
        try {
            const res = await XverServices.HospitalsWithMortalities(
            ...
            const sortedMortalities = res.data.sort((a: IMortalitiesDrillDown, b: IMortalitiesDrillDown) => {
                return b.death_rate - a.death_rate
            })
            setHospitals(sortedMortalities)
        } catch (err: any) {
            console.log(err)
            setLoading(prevState => ({
                ...prevState,
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className={`flex flex-center`}>
                                            ...
                                            >
                                                { getAnyValueShowToColumn(item.death_rate) }
                                            </Badge>
                                        </div>
                                        <p className={`text-center`}>
                                            { item.g00007 != null ? item.g00007 : '-' }
                                        </p>
                                        <p className={`text-center`}>
                                            {/* @ts-ignore */}
                                            { item.g00009 != null ? item.g00009 : '-' }
                                        </p>
                                    </div>
                                )
                            })
