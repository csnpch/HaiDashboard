...
import { IInfectionsDrillDown } from "@/interfaces/api/hospital"
...
export default function Infections(props: propsInterface) {
    ...
    const [hospitals, setHospitals] = useState<IInfectionsDrillDown[]>([])
    ...
    const fetchData = async () => {
        try {
            const res = await XverServices.HospitalsWithInfections(
            ...
            const sortedInfection = res.data.sort((a: IInfectionsDrillDown, b: IInfectionsDrillDown) => {
                if (!a.infection_rate.Valid || !b.infection_rate.Valid) return -1
                return b.infection_rate.Float64 - a.infection_rate.Float64
            })
            setHospitals(sortedInfection)
        } catch (err: any) {
            console.log(err)
            setLoading(prevState => ({
                ...prevState,
                                    ...
                                    >
                                        ...
                                        <div className={`flex flex-center`}>
                                            <Badge
                                                variant="filled"
                                                style={{
                                                    backgroundColor: item.infection_rate.Valid ? item.color : colors.N_A,
                                                    color: isBrightColor(item.color) ? 'black' : 'white',
                                                }}
                                            >
                                                { getAnyValueShowToColumn(
                                                    item.infection_rate.Valid
                                                    ? item.infection_rate.Float64.toFixed(2)
                                                    : 'N/A'
                                                ) }
                                            </Badge>
                                        </div>
                                        <p className={`text-center`}>
                                            { getAnyValueShowToColumn(item.ii_4_pcd) }
                                        </p>
                                        <p className={`text-center`}>
                                            {/* @ts-ignore */}
                                            { getAnyValueShowToColumn(item.G0002) }
                                        </p>
                                    </div>
                                )
                            })
